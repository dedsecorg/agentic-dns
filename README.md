...
| DNSCrypt | 5354 | DoH proxy | Cloudflare DoH |
...
# All DoT/DoH/DoQ from phone -> Pi-hole -> chain
```

Uses `nftables`/`iptables` TPROXY + `agentic-dns-server` DoT proxy to transparently intercept encrypted DNS (port 853, 443, 784) and force through Pi-hole.
