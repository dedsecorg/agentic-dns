...
struct Args {
    #[arg(short, long, default_value = "/etc/hermes-dns/config.toml")]
    config: String,
    ...
    #[arg(long)]
    dot_proxy: bool,
    #[arg(long, default_value = "/etc/hermes-dns/certs/dot.crt")]
    cert_file: String,
    #[arg(long, default_value = "/etc/hermes-dns/certs/dot.key")]
    key_file: String,
    #[arg(long, default_value = "100.74.31.18")]
    upstream_dns: String,
...
impl Default for AppConfig {
    fn default() -> Self {
        AppConfig {
            pihole_host: "100.74.31.18".to_string(),
            pihole_api_key: "AcKahJz1!!!!".to_string(),
            protonvpn_dns: "10.2.0.1".to_string(),
            services: vec![
                ...
                DnsService { name: "protonvpn-dns".into(), port: 53, addr: "10.2.0.1".into(), svc_type: "upstream".into(), upstream: None, fallback: None },
            ],
            alert_webhook: None,
...
fn get_chain() -> Vec<String> {
    vec![
        ...
        "protonvpn-dns (10.2.0.1) - final upstream via proton0".into(),
    ]
}

// ---- API Handlers ----
async fn api_status(Query(_): Query<HashMap<String, String>>) -> (StatusCode, Json<Value>) {
    let config = load_config("/etc/hermes-dns/config.toml");
...
async fn api_query(Query(params): Query<HashMap<String, String>>) -> (StatusCode, Json<Value>) {
    ...
    let addr = params.get("addr").cloned().unwrap_or_else(|| "100.74.31.18".to_string());
...
async fn api_trace(Query(params): Query<HashMap<String, String>>) -> (StatusCode, Json<Value>) {
    ...
    let services = discover_services(&config);
    let chain = get_chain();
    let result = dns_query(&domain, "100.74.31.18", 53);
    (StatusCode::OK, Json(json!({
        "domain": domain,
        "chain": chain,
        "services": services,
            ...
            if let Some(bypass_idx) = fo.find("bypassing ") {
                if let Some(arrow_idx) = fo.find(" -> ") {
                    ...
                    let _ = Command::new("bash")
                        .args(["/root/hermes-dns/bin/hermes-dns", "bypass", svc_name, backup_name])
                        .output();
                }
            }
        }
                ...
                "tools/call" => {
                    ...
                    let result_text = match tool_name {
                        ...
                        "dns_query" => {
                            ...
                            let addr = args.get("addr").and_then(|v| v.as_str()).unwrap_or("100.74.31.18");
                        ...
                        "dns_bypass" => {
                            ...
                            let output = Command::new("bash")
                                .args(["/root/hermes-dns/bin/hermes-dns", "bypass", service, backup])
                                .output();
                            match output {
                                Ok(out) => String::from_utf8_lossy(&out.stdout).to_string(),
                                Err(e) => format!("Error: {}", e),
